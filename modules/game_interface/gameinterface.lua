...
function init()
  ...
  connect(gameExtraRightPanel, { onVisibilityChange = onExtraRightPanelVisibilityChange })
...
function terminate()
  ...
  disconnect(gameExtraRightPanel, { onVisibilityChange = onExtraRightPanelVisibilityChange })
...
function tryExit()
  ...
  { { text=tr('Exit'), callback=exitFunc },
    ...
    anchor=AnchorRight}, logoutFunc, cancelFunc)
  return true
end
...
function tryLogout(prompt)
  ...
  if prompt then
    logoutWindow = displayGeneralBox(tr('Warning'), tr(msg), {
      ...
      anchor=AnchorRight}, yesCallback, noCallback)
  else
     yesCallback()
  end
end
  ...
  if g_keyboard.getModifiers() == KeyboardNoModifier then
	...
	local dire = smartWalkDir or dir
      if modules.client_options.getOption('smoothWalk') then
	    local snesivity = modules.client_options.getOption('walkingSensitivityScrollBar')
	    g_game.smoothWalk(dire, snesivity)
      else
	    g_game.walk(dire)
      end
    end
...
function moveStackableItem(item, toPos)
  ...
  if g_keyboard.isCtrlPressed() or g_keyboard.isKeyPressed("Enter") then
  ...
  g_keyboard.bindKeyDown('Enter', moveFunc)
  g_keyboard.bindKeyDown('Escape', cancelFunc)
...
function onExtraRightPanelVisibilityChange(extraRightPanel, visible)
  if not visible and g_game.isOnline() then
    local children = extraRightPanel:getChildren()
    for i=1,#children do
      children[i]:setParent(gameRightPanel)
    end
  end
...
function setupViewMode(mode)
  ...
  if currentViewMode == 2 then
    ...
    gameExtraRightPanel:setOn(modules.client_options.getOption('showExtraRightPanel'))
    gameLeftPanel:setImageColor('white')
    gameRightPanel:setImageColor('white')
    gameExtraRightPanel:setImageColor('white') 
    gameLeftPanel:setMarginTop(0)
    gameExtraRightPanel:setMarginTop(0)    
  ...
  elseif mode == 2 then
    ...
    gameExtraRightPanel:setImageColor('alpha')
    gameLeftPanel:setMarginTop(gameLeftPanel:getPaddingTop())
    gameRightPanel:setMarginTop(gameRightPanel:getPaddingTop())
    gameExtraRightPanel:setMarginTop(gameExtraRightPanel:getPaddingTop()) 
    gameExtraRightPanel:setOn(true)
    gameExtraRightPanel:setVisible(true)   
